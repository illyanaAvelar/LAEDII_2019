...
public class ArvoreSBB {
  ...
  private Item pesquisa (Item reg, No p) {
    if (p == null)//item não foi encintrado
    {
      comp++;//primeira comparação da função
      return null;
    } 
      else
        if (reg.compara (p.reg) < 0){//item menor, vai para subarvore esquerda
          comp += 2;//segunda comparação da função
          return pesquisa (reg, p.esq);
        }
        else 
          if (reg.compara (p.reg) > 0){//item maior, vai para subarvore direita 
            comp += 3;//terceira comparação da função
